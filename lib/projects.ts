...
export function findProjectBySlug(
  ...
  slug: string,
): PropertyProject | undefined {
  return projects.find((project) => getProjectSlug(project) === slug)
}

export function getConstructionStatus(
...
export function getConstructionStatusLabel(
  project: PropertyProject,
): string {
...
export function formatProjectPrice(project: PropertyProject): string {
  if (project.priceTo && project.priceTo > project.priceFrom) {
    return `от\u00A0${formatCompactPrice(project.priceFrom)} – до\u00A0${formatCompactPrice(project.priceTo)}`
  }

  return `от\u00A0${formatCompactPrice(project.priceFrom)}`
}
...
export function countPartnerProjects(
  partnerId: string,
  projects: PropertyProject[],
): number {
...
export function filterProjects(
  ...
  return projects.filter((project) => {
    if (
      filters.partnerId &&
      project.partnerId !== filters.partnerId
    ) {
      return false
    }
    ...
    if (
      filters.status !== 'all' &&
      getConstructionStatus(project) !== filters.status
    ) {
      return false
    }

    if (
      filters.priceRange === 'under30' &&
      project.priceFrom >= 30_000_000
    ) {
      return false
    }

    if (
      filters.priceRange === '30to40' &&
      (
        project.priceFrom < 30_000_000 ||
        project.priceFrom > 40_000_000
      )
    ) {
      return false
    }
...
export function getProjectHighlights(
  ...
  return items.length
    ...
    : [
        ...
        'Финансирование через Ailat Finance',
      ]
}
